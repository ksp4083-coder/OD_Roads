...
- 도로 주행 중 촬영된 10만장의 이미지와 각각의 이미지에 10종 객체의 위치 정보가 표기(라벨링)된 객체 위치 정보 파일(.json)
...
## 2. 개발 과정
### 2-1. 객체 위치 정보 정규화
- 객체 탐지 YOLO 모델을 불러와 Custome 데이터셋을 학습시키기 위해 객체 위치 정보 Bounding Box 값을 YOLO 형식인 0 ~ 1 사이의 값으로 변환
